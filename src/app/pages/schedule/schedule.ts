...
export class SchedulePage implements OnInit {
  ...
  public bannerImage: string =
    "../../../assets/img/main-event-program/default-main-event-banner.webp";
  ...
  extractDays() {
    ...
    this.expoSchedule.forEach((schedule) => {
    ...
    this.days = Array.from(daySet)
      ...
      .map((dayNum) => {
        ...
        const schedule = this.expoSchedule.find((s) => {
          const date = new Date(s.timeStart.seconds * 1000);
          return date.getDate() === dayNum;
        });
        return {
          label: `${schedule?.timeStart.toDate().toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
          })}`,
          value: dayNum,
